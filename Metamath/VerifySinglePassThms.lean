...
@[simp] theorem processFileSinglePass_zeroFuel
    ...
    processFileSinglePass fname processing seen config 0 s base =
      pure (.error (.cycleDetected s!"include depth limit exceeded in {fname}")) := by
  rfl
...
theorem checkerBridgeWitness_zeroIncludeDepth
  ...
  apply CheckerBridgeWitness.sharedIncludeError
    (err := .cycleDetected s!"include depth limit exceeded in {fname}")
