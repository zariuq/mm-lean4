...
The development comprises 54{,}130 lines of Lean across 46~files, with
zero \lean{sorry}s, zero axioms, and a clean build of 129~modules.
...
\item \textbf{End-to-end biconditional}
  ...
  configuration.
\item \textbf{Semantic bridge}
  (\leansmall{verify_parser_acceptance_iff_semantic_provable_total}):
  the same biconditional bridged to Carneiro's declarative
  \lean{Semantic.Provable}, using total database extraction.
...
The formal theorems cover \lean{checkBytes} given a \lean{ByteArray}.
Include expansion (\texttt{\$[ \ldots\ \$]}) is treated as a
pre-processing step outside the formalized core.
The verified part is: given fully expanded bytes, the parser correctly
checks every proof.
...
inductive Provable (axs : Statement -> Prop) (G : Context)
  ...
  | var (v:VR) : v.vhyp in G.hyps -> Provable axs G v
...
\begin{lstlisting}[caption={End-to-end biconditional (\file{KernelClean.lean}:10976).},label=lst:main]
...
\begin{lstlisting}[caption={Semantic bridge (\file{ParserEquivalence.lean}:441).},label=lst:semantic-bridge]
theorem verify_parser_acceptance_iff_semantic_provable_total
    ...
    (exists (fr : Spec.Frame),
      toFrame (Verify.checkBytes bytes)
            (Verify.checkBytes bytes).frame = some fr
      and Spec.Semantic.Provable
            (dbToAxioms (toDatabaseTotal (Verify.checkBytes bytes)))
            (frameToContext fr)
            (exprToFormula (varMapOfFrame fr) (toExpr f)))
\end{lstlisting}

Note that the right-hand side uses \lean{toDatabaseTotal}---a total
(non-\lean{Option}) database extraction.  The \lean{Option} wrapper in
the original \lean{toDatabase} was always \lean{some} by construction;
\lean{toDatabaseTotal} makes this explicit.
...
Underneath the parser-level biconditionals lies a pure specification-level
equivalence between the operational and declarative formulations of
provability.

\begin{lstlisting}[caption={Spec-level equivalence (\file{Spec/Equivalence.lean}:3505).},label=lst:equiv]
...
\paragraph{Premises.}
The theorem requires three structural well-formedness conditions:
...
\paragraph{Discharging from parser success.}
A specialized bridge theorem (\file{ParserEquivalence.lean})
discharges all three premises from a single hypothesis:
successful \lean{checkBytes} completion (no error).
The parser's well-formedness gate (\lean{wellFormed?}) and its
structural invariants guarantee that any successfully-parsed
database satisfies all three conditions.
Concretely, KernelClean's strong well-formedness extractor and
ParserEquivalence's total parser bridge package this discharge, so
reviewers do not need to re-thread generic premises at call sites.
See Listing~\ref{lst:parser-semantic} in Appendix~\ref{app:listings}.
...
The Metamath \texttt{\$[ \ldots\ \$]} include mechanism is handled
by IO-based pre-processing outside the formalized core.
Our theorems cover \lean{checkBytes} given a fully-expanded
\lean{ByteArray}.
...
\begin{itemize}[leftmargin=2em]
\item \textbf{Lean toolchain}: \texttt{leanprover/lean4:v4.18.0-rc1}
  (specified in \file{lean-toolchain})
\item \textbf{Dependencies}: Batteries v4.27.0-rc1 (no Mathlib)
\end{itemize}
...
\item \lean{operational_iff_semantic}: spec-level equivalence between
  operational and declarative provability.
\item \lean{parser_operational_iff_semantic_total}: specializes the
  bridge to parser-origin databases with total extraction.
...
\begin{itemize}[leftmargin=2em]
\item Formalizing include expansion (\texttt{\$[ \ldots\ \$]}),
  extending the trust boundary to cover file-level composition.
...
\subsection{\texttt{operational\_iff\_semantic} (Equivalence.lean:3505)}
...
\subsection{\texttt{parser\_operational\_iff\_semantic\_total} (ParserEquivalence.lean:380)}
...
theorem parser_operational_iff_semantic_total
    ...
    (h_frame : toFrame (Verify.checkBytes bytes)
                 (Verify.checkBytes bytes).frame = some fr) :
...
\subsection{\texttt{checkBytes\_parseErrorCode?\_fullyCertified} (ErrorCodeSemantics.lean:482)}
...
Table~\ref{tab:files} lists all 46 Lean files with line counts.

\begin{table}[ht]
\centering
\caption{Complete file inventory (54{,}130 lines total).}
\label{tab:files}
\small
\begin{tabular}{lr}
\toprule
...
Metamath/DBCaseAnalysis.lean & 2{,}006 \\
